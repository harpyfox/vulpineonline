...
async function loadSource(baseUrl: string): Promise<Blob | null> {
...
*/
function parseMasterPlaylist(playlist: string, baseUrl: string) {

    const parsed: { width: number, height: number, url: URL }[] = [];
...
#EXT-X-ENDLIST
*/
function parseLevelPlaylist(playlist: string, baseUrl: string) {
